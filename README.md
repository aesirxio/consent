...
import { ConsentReact } from 'aesirx-analytics';
const AnalyticsContainer = ({children}) => {
  const location = useLocation();
  let history = useHistory();
  return <ConsentReact location={location} history={history}>{children}</ConsentReact>;
};
...
import { ConsentNext } from "aesirx-analytics";

<ConsentNext router={useRouter()}>
  <[YOUR-COMPONENT]/>
</ConsentNext>
```
...
```
import { trackEvent, ConsentContext } from "aesirx-analytics";
const CustomEvent = () => {
  const AnalyticsStore = useContext(ConsentContext);
